...
func GetDiskStat() (stat []DiskStat, err error) {
	...
	for i := 0; i < len(lines); i++ {
		...
		if fields[0] != "none" && fields[0] != "proc" && fields[0] != "sysfs" && fields[0] != "devpts" && fields[0] != "tmpfs" {
...
func GetStat(path string) (stat DiskStat, err error) {
	...
	stat = DiskStat{}
	stat.Mount = path
	stat.Total = fs.Blocks * uint64(fs.Bsize) / 1024
	stat.Free = fs.Bfree * uint64(fs.Bsize) / 1024
	stat.Used = stat.Total - stat.Free
	stat.UsedRate = float64(stat.Used) * 100 / float64(stat.Total)
