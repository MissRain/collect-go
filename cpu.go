...
func GetCPUStat() (stat CPUStat, err error) {
	...
	stat.UserRate = float64(stat.User+stat.Nice) * 100 / float64(total)
	stat.SystemRate = float64(stat.System+stat.IRQ+stat.SoftIRQS) * 100 / float64(total)
	stat.IdleRate = float64(stat.Idle) * 100 / float64(total)
	stat.IOWaitRate = float64(stat.IOWait) * 100 / float64(total)
